...
import com.perevodchik.clickerapp.fromPercentage0
import com.perevodchik.clickerapp.loge
import com.perevodchik.clickerapp.logi
...
//        }
    ...
    private fun clear() {
        ...
        (getSystemService(Activity.ACTIVITY_SERVICE) as ActivityManager).killBackgroundProcesses(
            LIVE_ME_PACKAGE
        )
        "clear at ${Calendar.getInstance().time}".loge("STATE>FUNCTION")
    }
    ...
    fun start() {
        ...
        handler.postDelayed({
            ...
            startZero(iterator, null, id = ++cycleId)
        }, shared.getLong("extra_delay", 100L))
    }

    private fun startZero(
        i: ListIterator<Action>,
        _action: Action?,
        counter: Int = 0,
        id: Long
    ) {

        "cycleId $cycleId and id $id and isRunning $isRunning".logi("INFO")
        if (id != cycleId)
            return
        if (!isRunning) return
        if (_action != null && (counter > 1 || _action.loop)) {
            ...
            clickerService?.dispatchGesture(g.build(), object : GestureResultCallback() {
                override fun onCompleted(gestureDescription: GestureDescription?) {
                    ...
                    if (counter > 0 || id == cycleId)
                        startZero(i, _action, if (_action.loop) counter else counter - 1, id = id)
                }
            }, null)
            ...
            if (!action.enable) {
                ...
                clickerService?.dispatchGesture(g.build(), object : GestureResultCallback() {
                    override fun onCompleted(gestureDescription: GestureDescription?) {
                        super.onCompleted(gestureDescription)
                        if(id == cycleId)
                        startZero(i, action, action.repeat, id = id)
                    }
                }, null)
            }
        } else {
